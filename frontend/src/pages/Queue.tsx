...
import { PlusIcon, PlayIcon, CheckIcon, XMarkIcon, ClockIcon } from '@heroicons/react/24/outline';
...
const QueuePage: React.FC = () => {
  ...
  const handleSaveQueue = async (data: { patient_id: number; reason_for_visit: string }) => {
    try {
  ...
  const handleStatusChange = async (queueItem: Queue, newStatus: Queue['status']) => {
    try {
    ...
    <div className="space-y-6">
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{statistics.total_patients_today}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{statistics.waiting}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{statistics.served}</p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{statistics.skipped}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center space-x-4 mt-2">
                <span className="text-3xl font-bold text-blue-900">#{nowServing.queue_number}</span>
                <div>
                  <p className="font-medium text-blue-800">{nowServing.patient?.full_name}</p>
                  <p className="text-sm text-blue-600">{nowServing.reason_for_visit}</p>
                </div>
              </div>
            </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-gray-500">No patients in queue for {selectedDate}</p>
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        {queueItem.status === 'waiting' && (
                          <button
                            onClick={() => handleStatusChange(queueItem, 'serving')}
                        ...
                        {queueItem.status === 'serving' && (
                          <>
                            <button
                              onClick={() => handleStatusChange(queueItem, 'served')}
                            ...
                            <button
                              onClick={() => handleStatusChange(queueItem, 'skipped')}
