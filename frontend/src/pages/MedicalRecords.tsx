...
const MedicalRecords: React.FC = () => {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const response: PaginatedResponse<MedicalRecord> = await medicalRecordApi.getAll(
  ...
  const handleSaveRecord = async (data: Partial<MedicalRecord>) => {
    try {
      if (selectedRecord) {
        ...
        await medicalRecordApi.create(data as Omit<MedicalRecord, 'id' | 'created_at' | 'updated_at' | 'patient'>);
      }
                    ...
                    <tr key={record.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">
                          {record.patient?.full_name || getPatientName(record.patient_id)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(record.visit_date)}</div>
                      </td>
